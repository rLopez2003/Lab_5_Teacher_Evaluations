...
#This data set contains stuff that identifies teachers, the courses the students were in and the whether questions were asked in the class or if people participated in class. It also identifies the students gpa, the standard deviation, average and SET scores. It also tells us the percent failed of students the class duration and description of students.
...
#The teacher_id gives us the row of identification. 
```
...
teacher_evals_clean <- teacher_evals %>% 
  rename(sex = gender) %>%  #renamed sex to gender 
...
select(course_id, teacher_id, question_no, academic_degree, seniority, sex)
...
teacher_evals_clean %>%
  ...
  summarise(
    degree_counts = list(table(academic_degree)),
    seniority_range = paste0(min(seniority, na.rm = TRUE), " to ", max(seniority, na.rm = TRUE), " years"),
    sex_counts = list(table(sex))
  )
```
...
teacher_evals %>%
  ...
  group_by(teacher_id) %>%
  summarise(avg_rating_q1 = mean(SET_score_avg, na.rm = TRUE)) %>%
  arrange(desc(avg_rating_q1)) %>%
  slice_head(n = 1) %>%
...
teacher_evals_clean %>%
  filter(seniority == 1) %>%
  group_by(teacher_id) %>%
  summarise(avg_fail_pct = mean(percent_failed_cur, na.rm = TRUE)) %>%
  arrange(desc(avg_fail_pct)) %>%
  slice_head(n = 1) %>%
  bind_rows(
    teacher_evals_clean %>%
      filter(seniority == 1) %>%
...
teacher_evals_clean %>%
  ...
  group_by(teacher_id) %>%
  summarise(avg_response = mean(resp_share, na.rm = TRUE)) %>%
